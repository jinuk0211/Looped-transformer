...
Buck Shlegeris와 Ryan Greenblatt는 이것이 **CoT 모니터링의 유용성을 떨어뜨릴 수 있다**고 주장했다. 처리의 더 많은 부분이 보이지 않는 내부 연산으로 옮겨가면, 화면에 나타나는 설명만으로는 모델이 어떻게 결론에 도달했는지 파악하기 어려워질 수 있다는 것이다.
